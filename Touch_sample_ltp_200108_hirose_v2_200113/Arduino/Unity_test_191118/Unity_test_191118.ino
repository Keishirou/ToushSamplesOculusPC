...
#define SPEED2 5  // スライダを縮める場合5
#define SPEED3 180  // スライダを押しとどめる場合（仮想物体を下向きになぞっている際にめり込んだ場合）
#define SPEED4 200 //スライダを押しとどめるために伸ばす（仮想物体が変こんでいる場合）
...
void loop() {  
  if ( Serial.available() ) {
    ...
    if(index == -1){
      ...
      slide = recv_data[0].toInt();
      max_length_flag = recv_data[1].toInt();
      //blink(100);
    }
    //slide = str.toInt();
    //Serial.println("1");
