    ...
    void Update()
    {
        ...
        if (DtoO)
        {
            if (preTipD != null && preEndD != null)
            {
                ...
                {
                    nextTipD = tipD.transform.position + (tipD.transform.position - preTipD) * 3.0f;     // yama 180813 値が小さいため，情報落ちしないように次フレームのデバイス先端の座標を算出
                    nextEndD = endD.transform.position + (endD.transform.position - preEndD) * 3.0f;     // yama 180813 値が小さいため，情報落ちしないように次フレームのデバイス後端の座標を算出
        ...
        //if (Vector3.Distance(preTipD, tipD.transform.position) > 0.002)
        //{
        ...
        //}
        //if (Vector3.Distance(preEndD, endD.transform.position) > 0.002)
        //{
