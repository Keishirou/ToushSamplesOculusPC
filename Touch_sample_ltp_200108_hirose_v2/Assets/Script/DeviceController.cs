...
public class DeviceController : MonoBehaviour {
    ...
    public MeshDeformerInput_Sphere meshdeformer;
    int force_flag = 0;
    string str_flag;
    ...
    private void Update()
    {
        ...
        if (feedbackPattern == 0)       // yama 181220 力覚フィードバックで行う場合
        {
            if (sendFlag == 1)  // yama 180215 この判定で送信処理を行わないと，送信がバグる
            {
                ...
                if (slideMode == 0)         // yama 180808 伸縮予測を行わない場合
                {
                    ...
                    {
                        Debug.Log("DC1:" + sliderLength);
                        str = sliderLength.ToString();
                        str_flag = force_flag.ToString();   //hirose200109
                        //serialHandler.Write(str + ";");
                        serialHandler.Write(str + "," + str_flag + ";");    //hirose200109
                    }
                }
                else if (slideMode == 1)     // yama 180808 伸縮予測を行う場合
                {
                    if (0 <= nextLength && nextLength < 1024)          // yama 180807 伸縮予測用に変更 
                    {
                        ...
                        str_flag = force_flag.ToString();   //hirose 200109
                        //serialHandler.Write(str + ";");
                        serialHandler.Write(str + "," + str_flag + ";");    //hirose200109
        ...
        {
            slideMode = 0;
            //Debug.Log("slideMode = " + slideMode);    190909
        }
        ...
        //Debug.Log(feedbackPattern);       190909
    }
                    ...
                    if (sendFlag == 0)      // yama 180731 ここで判定している二つの条件（デバイス静止時，オブジェクトに接触）は同時に判定するとクラッシュする
                    {
                        if (DtoO == true)
                        {
                            ...
                            if (-JITTER < diff && diff < JITTER)        // yama 180719 スライダの位置ずれが一定範囲内かどうか判定
                            {
                                ...
                                if (slideMode == 0)
                                {
                                    force_flag = meshdeformer.send_force_flag();
                                    str_flag = force_flag.ToString();
                                    str = sliderLength.ToString();
    ...
    void Write_PreLength()
    {
        ...
        if (diff > (JITTER + 20) && diff < 500 && !DtoO)        // yama 181128 一定以上近づいている&デバイスが仮想物体に触れていない場合
        {
    ...
    // Rayを他のスクリプトに共有   // hirose 191030
    public Ray GetRay()
    {
        return ray;
    }

    // 床との衝突点を他のスクリプトに共有   // hirose 191030
    public Vector3 GetHitBase()
    {
        return hitF;
    }
    // デバイス先端を他のスクリプトに共有   // hirose 191030
    public GameObject GetDeviceTip()
    {
        return tipD;
    }
    // デバイス後端を他のスクリプトに共有   // hirose 191030
    public GameObject GetDeviceEnd()
    {
        return endD;
    }
}
