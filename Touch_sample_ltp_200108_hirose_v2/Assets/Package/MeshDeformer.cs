...
public class MeshDeformer : MonoBehaviour {
	...
	public float springForce = 20f; // 弾性力？　元：20f   springForce = k_min
	public float damping = 5f;      // 減衰   元：5f        damping = k_min/5
    ...
    [SerializeField]
    GameObject Sphere_End;
    MeshDeformerInput_Sphere mdis;
    ...
    void Start () {
        mdis = Sphere_End.GetComponent<MeshDeformerInput_Sphere>();
	...
	void Update () {
        /*
        if(mdis.send_hit_flag() == 1)
        {
            springForce = mdis.send_force() * 2;
            Debug.Log("弾性力："+ springForce);
        }
        */
    ...
    //変形する力を加える
	public void AddDeformingForce (Vector3 point, float force, float k) {
        springForce = k;
        damping = springForce / 4f;
        //Debug.Log("point" + point);
        //Vector3 device_tip = Device_Tip.transform.position;     // hirose 190822    デバイスの先端の座標を取得
        //Vector3 displacement_device = device_tip - point;     // hirose 190822 デバイスとオブジェクトの変位
        //float displacement_device = device_tip.magnitude - point.magnitude; // hirose 190822
