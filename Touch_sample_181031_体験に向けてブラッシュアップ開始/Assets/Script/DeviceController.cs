...
public class DeviceController : MonoBehaviour {
    ...
    public static int JITTER = 15;  // yama 181022 スライダの位置ずれの閾値
    ...
    // yama 181024 固定した秒数でループを行うFixedUpdate関数に変更
    void Update()
    //private void FixedUpdate() 
    {
            ...
            if (preTipD != Vector3.zero && preEndD != Vector3.zero)
            {
                if (preTipD != tipD.transform.position || preEndD != endD.transform.position)
                {
                    ...
                    for (int i = 0; i < preVecNum; i++)
                    {
                        ...
                        nextMotionTip += preTipVec[i] * (i + 1) / 15;
                        nextMotionEnd += preEndVec[i] * (i + 1) / 15;
                    }
        ...
        //Debug.Log("Fixed; hit.x = " + tipD.transform.position.x + ", hit.y = " + tipD.transform.position.y + ", hit.z = " + tipD.transform.position.z);

        //Vector3 pos = transform.TransformPoint(OVRInput.GetLocalControllerPosition(OVRInput.Controller.RTouch) + tipPosSet);
        //Debug.Log("Tip.x = " + tipD.transform.position.x + ", Tip.y = " + tipD.transform.position.y + ", Tip.z = " + tipD.transform.position.z);
        //Vector3 pos = tipD.transform.TransformPoint(tipD.transform.localPosition);
        //Vector3 pos = transform.TransformPoint(tipD.transform.localPosition);
        //Vector3 lpos = OVRInput.GetLocalControllerPosition(OVRInput.Controller.RTouch);
        //Debug.Log("Main.x = " + pos.x + ", Main.y = " + pos.y + ", Main.z = " + pos.z);
        //Debug.Log("Local.x = " + lpos.x + ", Local.y = " + lpos.y + ", Local.z = " + lpos.z);
        //Debug.Log("device.x = " + device.transform.position.x + ", Main.y = " + device.transform.position.y + ", Main.z = " + device.transform.position.z);
    ...
    void GetTips(GameObject obj)
    {
        if(obj.name.Equals(device_n + "_Tip"))
        {
            ...
            //tipD.transform.parent = (device.transform.parent.gameObject).transform;

            Debug.Log("Device_Tip position: " + tipD.transform.position);

            //tipPosSet = tipD.transform.localPosition - OVRInput.GetLocalControllerPosition(OVRInput.Controller.RTouch);
            //Debug.Log("tipLocal: x = " + tipD.transform.localPosition.x + ", y = " + tipD.transform.localPosition.y + ", z = " + tipD.transform.localPosition.z);
            tipPosSet = avatar.transform.InverseTransformPoint(tipD.transform.position);
            Debug.Log("tipPosSet: x = " + tipPosSet.x + ", y = " + tipPosSet.y + ", z = " + tipPosSet.z);
            //tipPosSet -= OVRInput.GetLocalControllerPosition(OVRInput.Controller.RTouch);
            //Debug.Log("tipPosSet: x = " + tipPosSet.x + ", y = " + tipPosSet.y + ", z = " + tipPosSet.z);
            //Vector3 lpos = OVRInput.GetLocalControllerPosition(OVRInput.Controller.RTouch);
            //Debug.Log("Local.x = " + lpos.x + ", Local.y = " + lpos.y + ", Local.z = " + lpos.z);
            //Debug.Log("worldToLocalMatrix = " + tipD.transform.worldToLocalMatrix);
    ...
    {     
        if (sendFlag == 0)  // yama 180830 送信待ち状態のときのみ計算（理由は実装履歴参照）
        {
            if (DtoO)    // yama 180214 デバイスが仮想物体に接触しているか（レイではなくコライダが）
            {
                ...
                int dis = 1024 - (int)(disOtoF* 1.07 / 0.5 * 1024);  // yama 180214 距離をスライダに合わせて正規化（0214時点では 1unit = 20cm）
                                                               //Debug.Log("dis = " + dis);
